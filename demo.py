...
def main(args):
    ...
    if args.demo_type == "location":
        ...
        obs_time = (
            torch.ones(coords.shape[0], device=params["device"]) * args.time_of_year * 2
            - 1.0
        )
        loc_time_feats = utils.encode_loc_time(
            obs_coords, obs_time, concat_dim=1, params=params
        )
        ...
        for ii, cls_id in enumerate(most_likely[:num_categories]):
            print(
                "{}\t{}\t{:.3f}".format(ii, cls_id, np.round(pred[cls_id], 3))
                + "\t"
                + class_data[cls_id]["our_name"]
                + " - "
        ...
        gp = grid.GridPredictor(
            np.load("data/ocean_mask.npy"), params, mask_only_pred=True
        )
        ...
        print("Making prediction ...")
        grid_pred = gp.dense_prediction(
            model, args.class_of_interest, time_step=args.time_of_year
        )

        op_file_name = (
            class_data[args.class_of_interest]["our_name"].lower().replace(" ", "_")
            + ".png"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "demo_type", type=str, help='Can be either "map" or "location".'
    )
    parser.add_argument(
        "--model_path", type=str, default=model_path, help="Path to trained model."
    )
    parser.add_argument(
        "--model_url", type=str, default=model_url, help="Path to remote trained model."
    )
    parser.add_argument(
        "--class_names_path",
        type=str,
    ...
    parser.add_argument(
        "--longitude", type=float, default=-118.1445155, help="Longitude of interest."
    )
    parser.add_argument(
        "--latitude", type=float, default=34.1477849, help="Latitude of interest."
    )
    parser.add_argument(
        "--time_of_year", type=float, default=0.5, help="Time of year [0, 1]."
    )
    parser.add_argument(
        "--class_of_interest",
        type=int,
